...
export function OperationsExplorer() {
  ...
  return <section className="py-16"><p className="font-mono text-xs uppercase tracking-[.2em] text-primary">{t.eyebrow}</p><h2 className="text-balance mt-4 max-w-4xl text-4xl font-medium leading-[1] tracking-[-.04em] sm:text-5xl">{t.title}</h2><Tabs.Root value={active} onValueChange={setActive} className="mt-10 overflow-hidden rounded-[1.5rem] border border-border bg-white/[.03]"><Tabs.List className="flex overflow-x-auto border-b border-border">{t.tabs.map((tab, index) => <Tabs.Trigger key={tab.id} value={tab.id} className="group relative min-w-36 flex-1 px-5 py-5 text-left text-sm text-muted-foreground outline-none transition-colors data-[state=active]:text-foreground"><span className="mr-3 font-mono text-[9px] text-primary">0{index + 1}</span>{tab.label}<span className="absolute inset-x-0 bottom-0 h-px scale-x-0 bg-primary transition-transform group-data-[state=active]:scale-x-100" /></Tabs.Trigger>)}</Tabs.List><div className="grid lg:grid-cols-[1.35fr_.65fr]"><div className="min-h-[31rem] overflow-hidden border-b border-border p-6 lg:border-b-0 lg:border-r sm:p-10"><AnimatePresence mode="wait"><motion.div key={selected.id} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: .35 }}><div className="flex flex-col justify-between gap-10 sm:flex-row"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">{selected.metricLabel}</p><p className="mt-3 text-4xl font-medium tracking-[-.05em] text-primary">{selected.metric}</p></div><Popover trigger={<button className="self-start rounded-full border border-border px-4 py-2 text-xs hover:border-primary">{t.details} +</button>}><p className="leading-6 text-muted-foreground">{t.detailBody}</p></Popover></div><div className="mt-14"><h3 className="text-3xl font-medium tracking-[-.04em]">{selected.headline}</h3><p className="mt-5 max-w-xl leading-7 text-muted-foreground">{selected.body}</p><div className="mt-8 grid gap-px bg-border sm:grid-cols-3">{selected.signals.map((signal) => <div key={signal} className="bg-background p-4 text-xs"><span className="mr-2 text-primary">●</span>{signal}</div>)}</div></div></motion.div></AnimatePresence></div><div className="p-6 sm:p-8"><p className="mb-5 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">SYSTEM NOTES</p><Accordion items={[...t.faq]} /></div></div></Tabs.Root></section>;
}
